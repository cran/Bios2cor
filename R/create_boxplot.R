...
#
create_boxplot <- function(correlation_objects, objects_names, elite, high, filepath){
  corr_tab <- sapply(correlation_objects, function(x){ as.vector(x)})
  nb_objects <- length(correlation_objects)
  
  print(paste("Boxplot elements :", nb_objects))
  
  png(filepath, width = 400, height = 800, units = "px", pointsize = 12)
    boxplot(corr_tab, names = objects_names, xlab="Z-score", cex.lab=1.5, cex.axis=0.90, col = "grey", outcol = "grey", horizontal=T, las=1)
